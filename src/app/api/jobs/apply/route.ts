...
export async function POST(req: Request) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      console.log('APPLY API FORM ENTRIES:', entries);
      ...
      if (resumeFile && resumeFile.size > 0) {
        // Basic validation: limit size to 5MB and allowed mime types
        ...
        const safeName = `${Date.now()}-${payload.userId}-${jobId || '0'}-${originalName}`.replace(/[^a-zA-Z0-9.\-_]/g, '_');
    ...
    const existing = await db
      .select()
      .from(applications)
      .where(eq(applications.jobId, jobId), eq(applications.userId, payload.userId), isNull(applications.deletedAt));
    ...
    // Verify job exists
    const [job] = await db.select().from(jobs).where(eq(jobs.id, jobId), isNull(jobs.deletedAt));
    if (!job) {
    ...
    // Insert application
    await db.insert(applications).values({ jobId, userId: payload.userId, status: 'pending', coverLetter: coverLetter ?? null, resumePath: resumePath ?? null });

    // Atomically increment applicants count
    await db.update(jobs).set({ applicants: sql`${jobs.applicants} + 1` }).where(eq(jobs.id, jobId));

    // Fetch updated applicants count
    const [updatedJob] = await db.select().from(jobs).where(eq(jobs.id, jobId));
    const updatedCount = updatedJob?.applicants ?? null;

    return NextResponse.json({ status: 'success', message: 'Application submitted', data: { jobId, applicants: updatedCount } });
  } catch (error) {
