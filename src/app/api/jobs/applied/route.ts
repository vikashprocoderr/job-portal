...
export async function GET(req: Request) {
  try {
    ...
    const rows = await db.select({ jobId: applications.jobId }).from(applications).where(eq(applications.userId, payload.userId), isNull(applications.deletedAt));
    const jobIds = rows.map(r => Number(r.jobId));
    return NextResponse.json({ status: 'success', data: jobIds });
  } catch (error) {
    console.error('Error fetching applied jobs:', error);
    return NextResponse.json({ status: 'error', message: 'Failed to fetch applied jobs' }, { status: 500 });
  }
}
